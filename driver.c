 ...
 * would provide some kind of hardware FIFO to transmit the message, and interrupt
 * us upon reception of a complete CEC frame. It does not support hardware counters
 * nor detaching from host.
 */
...
static int cec_example_send(struct cec_driver *drv, const u8 *data, const u8 len)
{
	struct cec_example_priv *priv =
		container_of(drv, struct cec_example_priv, cec_drv);
